...
for path in game_paths:
    with open(path) as f:
        ...
        if 'support_language' not in data_dict.keys():
            support_language = None
        else:
            support_language = data_dict['support_language']

        print()

        # insert into the game list
        conn.execute("INSERT INTO backend_gamelist (game_id, game_name, image_url, game_description, support_language, required_age, developer, publisher, linux, mac, windows, price, _order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (game_id,
                game_name,
                header_image,
